...
function sakura_widget_quickflickr($args) {
	...
	{	
		...
		if (empty($items) || $items < 1 || $items > 24) $items = 4;
...
function sakura_widget_quickflickr_control() {
	...
	if ( isset($_POST["sakura-flickr-submit"]) && $_POST["sakura-flickr-submit"] ) {
        ...
        if( isset($_POST["sakura-rss-items"]) && ($_POST["sakura-rss-items"]*1) > 0 && ($_POST["sakura-rss-items"]*1) < 24 ) {
	...
	if( empty($items) || $items < 1 ) $items = 4;
