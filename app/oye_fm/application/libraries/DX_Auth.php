...
class DX_Auth {
    ...
    function _increase_login_attempt($user) {
        if ($this->ci->config->item('DX_count_login_attempts') AND ! $this->is_max_login_attempts_exceeded()) {
            ...
            $this->ci->login_attempts->increase_attempt($this->ci->input->ip_address(), $user);
        }
    }
        ...
        if ($this->is_logged_in()) {
            ...
            if (!$this->is_admin()) {
                ...
                if (!$have_access) {
                    ...
                    $url = $this->deny_access();
                    redirect(base_url('usuarios' . $url), 'refresh');
//                    $this->deny_access();
                }
            }
        } else {
            $url = $this->deny_access('login');
            redirect(base_url('usuarios' . $url), 'refresh');
...
//            $this->deny_access('login');
    ...
    function deny_access($uri = 'deny') {
        ...
        $url = '';
        if ($uri == 'login') {
            $url = $this->ci->config->item('DX_login_uri');
        } else if ($uri == 'banned') {
            $url = $this->ci->config->item('DX_banned_uri');
        } else {
            $url = $this->ci->config->item('DX_deny_uri');
        }
        return $url;
    }
