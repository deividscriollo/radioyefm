...
class Backend extends MY_Controller {
    ...
    function users() {
        ...
        $p_config = $this->config_pagination($row_count);
        // Init pagination
...
//        $this->load->view('backend/users', $data);
        $this->loadTemplates('backend/users', $data);
    }
    ...
    function roles() {
        ...
        if ($this->input->post('add')) {
            // Create role
            if ($this->input->post('role_name') !== '' && $this->input->post('role_name') !== NULL)
        ...
        // Load view
        $this->loadTemplates('backend/roles', $data);
//        $this->load->view('backend/roles', $data);
    ...
    protected function config_pagination($row_count) {
        $p_config['full_tag_open'] = '<ul class="pagination pagination-md">';
        $p_config['full_tag_close'] = '</ul>';
        $p_config['num_tag_open'] = '<li>';
        $p_config['num_tag_close'] = '</li>';
        $p_config['cur_tag_open'] = '<li class="active"><span>';
        $p_config['cur_tag_close'] = '<span></span></span></li>';
        ...
        $p_config['base_url'] = base_url('usuarios/backend/users/');
        $p_config['uri_segment'] = 4;
        $p_config['num_links'] = 20;
        $p_config['total_rows'] = $this->users->get_all()->num_rows();
        $p_config['first_link'] = 'Primera';
        $p_config['last_link'] = 'Última';
        $p_config['next_link'] = 'Siguiente';
        $p_config['prev_link'] = 'Anterior';
        $p_config['per_page'] = $row_count;
        return $p_config;
    }
