        ...
        function ($scope, $http, $modal, $log, $filter, ngTableParams, MessageBox) {
            ...
            function filter_by_fields(data) {
                ...
                if (text) {
                    ...
                    return !text || reg_exp.test(data.nombre) || reg_exp.test(data.apellido)
                            || reg_exp.test(data.cedula) || reg_exp.test(data.edad)
                            || reg_exp.test(data.direccion) || reg_exp.test(data.telefono)
                            || reg_exp.test(data.tipo_vendedor.text) || reg_exp.test(data.observaciones);
                }
                return true;
            }
            ...
            $scope.showModal = function (fila) {
                var modalInstance = $modal.open({
                    templateUrl: appOye_fm.urlModal + '/modalVendedor.html',
            ...
            var ModalInstanceCtrl = function ($scope, $modalInstance, items) {
                ...
                $scope.getTipoVendedor = function (term, done) {
                    $http.get(appOye_fm.urlGetTipoVendedor).success(function (result, status) {
                        done($filter('filter')(result, {text: term}, 'text'));
                    });
                };

                $scope.valida_cedula = function (cedula) {
                    if (cedula !== undefined && cedula !== "") {
                        if (!check_cedula(cedula))
                            items.cliente.cedula = "";
                    }
                };
            };
            ...
            function procesar(registro, $modalInstance) {
                ...
                validar_repetido(id, registro.cliente.cedula, function (message) {
                    if (message == "ok") {
